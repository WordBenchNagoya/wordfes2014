...
class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$menu_icons = get_post_meta( $item->ID, 'menu-icons', true ) ? get_post_meta( $item->ID, 'menu-icons', true ) : array() ;
		/**
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			if ( !empty( $menu_icons['type'] ) ) {
				$type = ( isset( $menu_icons['type'] ) ? $menu_icons['type']  : '' ) . '-icon';
				$title = sprintf(
					'%s<i class="_mi _%s %s %s"%s></i>%s',
					'before' === $menu_icons['position'] ? '' : $item->title,
					esc_attr( $menu_icons['position'] ),
					esc_attr( ( isset( $menu_icons['type'] ) ? $menu_icons['type']  : '' ) ),
					esc_attr( ( ( $type !== '-icon'  ) ? $menu_icons[ $type ] : '' ) ),
					$this->get_style( $menu_icons ),
					'after' === $menu_icons['position'] ? '' : $item->title
				);
			} else {
				$title = $item->title;
			}
			...
			if ( ! empty( $item->attr_title ) )
				$item_output .= '<a'. $attributes .'><span class="glyphicon ' . esc_attr( $title  ) . '"></span>&nbsp;';
			else
				$item_output .= '<a'. $attributes .'>';

			$item_output .= $args->link_before . apply_filters( 'the_title', $title , $item->ID ) ;
			$item_output .= ( isset( $item->post_content ) ? '<span class="nav-desc">' . $item->post_content . '</span>' . $args->link_after :  $args->link_after );
	 ...
	 * @return string
	 */
	public function get_style( $values ) {
		$style = '';
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">メニューを追加</a></li>';
			$fb_output .= '</ul>';
