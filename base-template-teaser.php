<?php
/*
Template Name: ティザー テンプレート
*/
/**
 * ティザーサイト テンプレート
 * =====================================================
 * @package    Wordfes2014
 * @author     WordBench Nagoya
 * @license    http://opensource.org/licenses/MIT
 * @link       http://2014.wordfes.org
 * @copyright  2014 WordBench Nagoya
 * =====================================================
 */
?>
<!DOCTYPE html>
<html class="no-js" <?php language_attributes(); ?>>
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black">
<meta name="apple-mobile-web-app-title" content="WordFes Nagoya 2014">
<meta name="msapplication-TileColor" content="#3372DF">
<?php wp_head(); ?>
<link rel="alternate" type="application/rss+xml" title="<?php echo get_bloginfo( 'name' ); ?> Feed" href="<?php echo esc_url( get_feed_link() ); ?>">
</head>
<body <?php body_class() ?> onload="init();">
	<header class="header">
		<div class="container">
			<div class="social-area">
				<div class="sns-button fb-like" data-href="http://2014.wordfes.org" data-layout="button_count" data-action="like" data-show-faces="true" data-share="false"></div>
				<div class="sns-button">
					<a href="https://twitter.com/share" data-url="http://2014.wordfes.org" class="twitter-share-button" data-via="wordbenchnagoya" data-lang="">ツイートする</a>
				</div>
			</div>
	    <div class="navbar-header">
	      <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
	        <span class="sr-only">Toggle navigation</span>
	        <span class="icon-bar"></span>
	        <span class="icon-bar"></span>
	        <span class="icon-bar"></span>
	      </button>
	    </div>
	    <nav class="collapse navbar-collapse" role="navigation">
				<?php
				 if ( has_nav_menu( 'primary_navigation' ) ) :
						wp_nav_menu(
							array(
								'theme_location' => 'primary_navigation',
								'menu_class' => 'nav-main nav navbar-nav',
								'fallback_cb'       => 'WP_Bootstrap_Navwalker::fallback',
								'walker'            => new WP_Bootstrap_Navwalker()
							 )
						 );
				 endif;
				?>
			</nav>
		</div>
	</header>
	<div id="main_area" style="opacity:0;">
		<div class="container">
			<div class="clearfix">
				<div class="col-md-6 main_logo">
					<h1><img src="data:image/png;base64," alt="WordFes Nagoya 2014 愛がある！冒険がある！WordPressがある！"></h1>
					<p class="date"><span>2014.<strong>8.30</strong>(SAT)</span>名古屋工業大学52号館</p>
					<p>WordFesとして2年目を迎える今年は、
					「愛がある、冒険がある、WordPressがある」をテーマにしました。
					WordPressやコミニティを通じての出逢いや経験に感謝のキモチと愛をこめて、
					そして、このWordFesで新しい発見や気付きが生まれて冒険したくなるような、
					ワクワクするイベントになればと思います。</p>
				</div>
				<div class="col-md-6 hidden-xs" id="livestage">
					<div class="slider">
						<img src="data:image/png;base64,">
					</div>
					<span id="stage_light"><img src="data:image/png;base64,"></span>
					<span id="stage_speaker1"><img src="data:image/png;base64,"></span>
					<span id="stage_staff"><img src="data:image/png;base64,"></span>
				</div>
			</div>
		</div><!-- /container -->

	</div><!-- /#main_area -->


	<section id="schedule">
		<div class="container">
			<div class="clearfix">
				<h2>SCHEDULE</h2>
				<section class="col-md-4" id="conference">
					<h3><i class="glyphicon glyphicon-pencil"></i><span>CONFERENCE</span>セッション</h3>
					<div class="inner">
						<p class="text-center time"><strong>8.30</strong>(SAT)<em>11:00-17:00</em></p>
						<table class="table table-bordered">
							<tr>
								<th>場所</th>
								<td>名古屋工業大学 52号館　<a href="http://maps.google.com/maps?q=35.158406,136.924918" target="_blank" class="linkicon">MAP</a></td>
							</tr>
							<tr>
								<th>料金</th>
								<td>0円（無料）</td>
							</tr>
							<tr>
								<th>定員</th>
								<td>150名</td>
							</tr>
						</table>
					</div>
				</section>
				<section class="col-md-4" id="party">
					<h3><i class="glyphicon glyphicon-cutlery"></i><span>PARTY</span>セッション + 懇親会</h3>
					<div class="inner">
						<p class="text-center time"><strong>8.30</strong>(SAT)<em>17:30-19:30</em></p>
						<table class="table table-bordered">
							<tr>
								<th>場所</th>
								<td>浩養園〈こうようえん〉　<a href="http://maps.google.com/maps?q=浩養園" target="_blank" class="linkicon">MAP</a></td>
							</tr>
							<tr>
								<th>料金</th>
								<td>4,000円</td>
							</tr>
							<tr>
								<th>定員</th>
								<td>52名</td>
							</tr>
						</table>
					</div>
				</section>
				<section class="col-md-4" id="stay">
					<h3><i class="glyphicon glyphicon-briefcase"></i><span>STAY &amp; MEETING</span>セッション + 懇親会 + 宿泊</h3>
					<div class="inner">
						<p class="text-center time"><strong>8.30</strong>(SAT)<em>21:00 - </em><strong>8.31</strong>(SUN)</p>
						<table class="table table-bordered">
							<tr>
								<th>場所</th>
								<td>新舞子まなビレッジ南浜荘　<a href="http://maps.google.com/maps?q=新舞子まなビレッジ南浜荘" target="_blank" class="linkicon">MAP</a></td>
							</tr>
							<tr>
								<th>料金</th>
								<td>8,000円（懇親会費含む）</td>
							</tr>
							<tr>
								<th>定員</th>
								<td>50名</td>
							</tr>
						</table>
					</div>
				</section>
			</div>
		</div>
	</section><!-- /schedule -->

	<section class="text-center background-color beige">
		<div class="container">
			<a class="hover_focus" href="<?php echo site_url( '/sponsor/' ) ?>"><img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/img/subpage/sponsor_bn.png" alt="ご協賛いただけるスポンサー様募集中"></a>
		</div>
	</section>
	<div class="responsive_canvas">
		<canvas id="canvas" width="1980" height="730"></canvas>
	</div>

<?php

get_template_part( 'modules/footer' );
